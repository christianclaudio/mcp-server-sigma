...
echo "[*] Starting mcp-server-sigma server on ${HOST}:${PORT} (streamable-http)..."
SIGMA_CLIENT_ID="ci-placeholder-id" SIGMA_CLIENT_SECRET="ci-placeholder-secret-32-chars-long" "${RUN_CMD[@]}" -m sigma_mcp.server --transport streamable-http --host "${HOST}" --port "${PORT}" &
SERVER_PID=$!
